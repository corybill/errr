...
import { formatDebugParams } from "./format-debug-params.js";
import { maybeCleanErrrStack } from "./stack-clean.js";

// `class Error` below shadows the global `Error` inside this module, so capture a reference
// to the native one for use in `Private.toError`.
 ...
 */
function defineHiddenErrorProp(error, key, value) {
...
class Private {
  ...
  /**
   * Normalize an arbitrary value (e.g. an `unknown` from a TypeScript strict catch) into something safe
   * to append to a stack trace. Error instances and error-like objects (with string `.stack` and `.message`)
   * pass through unchanged so their original stack is preserved; everything else is wrapped in
   * `new Error(...)` so callers don't need to write a per-call `toError` helper.
   *
   * @param {*} value - Arbitrary value (typically a caught error, but may be a string, object, etc.).
   * @returns {Error} - An Error (or error-like) suitable for appendTo.
   */
  static toError(value) {
    ...
    if (typeof value === "string") {
      return new NativeError(value);
    }

    return new NativeError(util.inspect(value, { depth: 5, compact: true, breakLength: Infinity }));
  }
}
...
class Error {
   ...
   * @returns {ErrrorrrBuilder} - Returns the instance of errorBuilder to allow chainability.
   */
   ...
   *
   * @param {*} err - Error, error-like object, or any other value to append. The stack trace of the
   * error being built will be appended to this error's stack trace.
   * @returns {ErrrorrrBuilder} - Returns the instance of errorBuilder to allow chainability.
   */
  appendTo(err) {
    ...
    this._appendTo_.push(Private.toError(err));
    return this;
  }
  ...
  _build_(error) {
    error.stack = (!this._debugParams_) ? error.stack :
      util.format(constants.templates.StackDebugParamsTemplate, error.stack, formatDebugParams(this._debugParams_));
    ...
    for (let currentAppendTo of this._appendTo_) {
      ...
      if (currentAppendTo && currentAppendTo._setValues_) {
        for (const key in currentAppendTo._setValues_) {
          if (Object.prototype.hasOwnProperty.call(currentAppendTo._setValues_, key)) {
    ...
    for (const key in this._setValues_) {
      if (Object.prototype.hasOwnProperty.call(this._setValues_, key)) {
    ...
    if (error._allDebugParams_ !== undefined) {
      defineHiddenErrorProp(error, "_allDebugParams_", error._allDebugParams_);
    }
    defineHiddenErrorProp(error, "_setValues_", error._setValues_);
    defineHiddenErrorProp(error, "set", error.set);
    defineHiddenErrorProp(error, "get", error.get);
    defineHiddenErrorProp(error, "getAllDebugParams", error.getAllDebugParams);

    error.stack = maybeCleanErrrStack(error.stack);
